...
import json
import os
from ..tools import node_filter
import sys


# 設定ファイルのパス
SETTINGS_DIR = os.path.expanduser("~/.ez_modeling_tools")
SETTINGS_FILE = os.path.join(SETTINGS_DIR, "node_filter_settings.json")
...
class NodeListDialog(QtWidgets.QDialog):
    ...
    def on_refresh(self):
        ...
        # フレーズ設定を収集
        from ..tools import node_filter as nf
...
class NodeFilterWidget(QtWidgets.QWidget):
    """Node Filter ツールウィジェット"""
    ...
    def __init__(self, parent=None):
        ...
        # ウィンドウフラグを設定
        self.setWindowFlags(QtCore.Qt.Window)
        self.setWindowTitle("Node Filter")
    ...
    def on_refresh(self):
        ...
        if use_common_filter and common_include_configs:
            # 共通フィルターで最初のフィルタリング（常に'all'モード - すべての共通フィルターに一致）
            matched_nodes = node_filter.filter_nodes_by_phrase_configs(common_include_configs, 'all')
        else:
            # 共通フィルターがない、または使用しない場合はすべてのノードから開始
            matched_nodes = node_filter.get_all_nodes()

        # 2. 共通フィルターの除外を適用（共通フィルター使用が有効な場合のみ）
        if use_common_filter and common_exclude_configs:
            common_excluded_nodes = node_filter.filter_nodes_by_phrase_configs(common_exclude_configs, 'any')
            matched_nodes = [node for node in matched_nodes if node not in common_excluded_nodes]
        ...
        if include_configs:
            # 一時的にすべてのノードを取得する関数を上書き
            original_get_all = node_filter.get_all_nodes
            node_filter.get_all_nodes = lambda: matched_nodes

            matched_nodes = node_filter.filter_nodes_by_phrase_configs(include_configs, match_mode)

            # 元に戻す
            node_filter.get_all_nodes = original_get_all

        # 4. フレーズプリセットの除外フィルタリング実行（除外フレーズがある場合）
        if exclude_configs:
            # 除外フレーズに一致するノードを取得（除外は常に'any'モード）
            excluded_nodes = node_filter.filter_nodes_by_phrase_configs(exclude_configs, 'any')
            # 包含リストから除外リストを削除
            matched_nodes = [node for node in matched_nodes if node not in excluded_nodes]
    ...
    def on_select_nodes(self):
        ...
        # 選択実行
        node_filter.select_nodes(nodes)
    ...
    def load_settings(self):
        ...
        try:
            if not os.path.exists(SETTINGS_FILE):
                # デフォルトで階層を作成
                print("Node Filter: 設定ファイルが存在しないため、デフォルト階層を作成します")
            ...
            if version == 2:
                ...
                print(f"設定を読み込みました（階層構造）: {SETTINGS_FILE}")

            else:
                # 旧形式からマイグレーション
                print("Node Filter: 旧形式の設定を階層構造に変換します")
                    ...
                    for phrase_index, phrase_preset in enumerate(work_data.get('phrase_presets', [])):
                        ...
                        if phrase_preset.get('dialog_open', False):
                            ...
                            if all_include_configs:
                                match_mode = phrase_preset.get('match_mode', 'any')
                                matched_nodes = node_filter.filter_nodes_by_phrase_configs(all_include_configs, match_mode)
...
def create_node_filter_tab(parent=None):
    """
    Node Filterタブを作成
    ...
    Returns:
        NodeFilterWidget: 作成されたウィジェット
    """
    return NodeFilterWidget(parent)
